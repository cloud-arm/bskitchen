...
if($type == 'Return'){
    ...
    $result->execute();
    for($i=0; $row = $result->fetch(); $i++){$pro = $row['product_id']; $pro_name = $row['name']; }
}else{

    $result = $db->prepare("SELECT * FROM products WHERE id=:id ");
    $result->bindParam(':id', $pro);
    $result->execute();
    for($i=0; $row = $result->fetch(); $i++){$pro_name = $row['product_name']; }
}
...
if($invo!=''){
    $sql = "INSERT INTO purchases_list (invoice_no,name,qty,amount,date,product_id,cost,sell,discount,type,user_id,stock_id) VALUES (?,?,?,?,?,?,?,?,?,?,?,?)";
    $re = $db->prepare($sql);
